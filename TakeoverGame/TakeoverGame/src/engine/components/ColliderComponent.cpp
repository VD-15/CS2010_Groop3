...
namespace
{
	...
	void OnLateUpdate(LateUpdateEvent& ev)
	{
		...
		for (auto first = colliders2D.begin(); first != colliders2D.end(); first++)
		{
			for (auto second = first + 1; second != colliders2D.end(); second++)
			{
				...
				for (auto it = axes.begin(); it != axes.end(); it++)
				{
					...
					c1->GetProjection(*it, range1.start, range1.end);
					c2->GetProjection(*it, range2.start, range2.end);

					if (vlk::Compare(range1.end, range2.start) == vlk::Compare(range1.start, range2.end))
					{
						colliding = false;
						break;
					}
		...
		{
			EventBus<CollisionEvent2D>::Get().PostEvent(CollisionEvent2D(it->first, it->second));
		}
	}
}
